<script>
  ...
  let colorClass;
  $: colorClass = block.aLV >= 2 ? 'bg-red-500/75' : block.aLV >= 1 ? 'bg-orange-500/75' : 'bg-slate-700';
  let editing = false;
</script>


<div class="flex overflow-hidden text-xs rounded-lg shadow bg-slate-800">
  <div class="w-1 {colorClass}"></div>
    ...
    <div class="px-2 pt-1">
      ...
      <div class="flex gap-1 text-sm">
        <div class="relative flex-1">
          <textarea bind:value={block.oStrs[field]} class="w-full h-20 p-1 rounded shadow-sm outline-none resize-none text-slate-400 bg-slate-900/25" readonly></textarea>
